...
import type { Fit, GpuInfo, HardwareProfile, HfFile, HfModel, LocalModel } from "@/types";
      ...
      {loading && !rec ? (
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="space-y-1">
              ...
              <TextInput
                ...
                step={1024}
                value={contextLen}
                onChange={(e) => setContextLen(e.target.value ? Number(e.target.value) : "")}
              />
            </div>
            <div className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 rounded-lg border border-border bg-background/40 p-3 text-xs sm:grid-cols-3">
            ...
            <Stat label="Est. VRAM" value={rec.est_vram_mb ? `${rec.est_vram_mb} MB` : "—"} />
            <Stat label="Est. RAM" value={rec.est_ram_mb ? `${rec.est_ram_mb} MB` : "—"} />
...
function DiscoverTab({ onConfigure }: { onConfigure: (t: ConfigTarget) => void }) {
  const { downloadModel, localModels } = useApp();
  ...
  // Per-file download state, keyed by `${repo}/${path}`.
  const [downloads, setDownloads] = useState<
    Record<string, { downloaded: number; total: number; status: "downloading" | "done" | "error"; message?: string }>
  >({});
  ...
  const download = async (repoId: string, file: HfFile) => {
    const key = `${repoId}/${file.path}`;
    setDownloads((prev) => ({ ...prev, [key]: { downloaded: 0, total: file.size_bytes, status: "downloading" } }));
    try {
      await downloadModel({ repo_id: repoId, file_path: file.path, quant: file.quant }, (event) => {
        setDownloads((prev) => {
          if (event.type === "progress") {
            return { ...prev, [key]: { downloaded: event.downloaded, total: event.total, status: "downloading" } };
          }
          if (event.type === "done") {
      ...
      setDownloads((prev) => ({ ...prev, [key]: { downloaded: 0, total: 0, status: "error", message: String(e) } }));
    }
  };
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {have ? (
                                ...
                                <Button
                                  ...
                                  onClick={() => download(repo.repo_id, file)}
...
/** Library: the downloaded GGUFs — configure (launch command) or remove. */
function LibraryTab({ onConfigure }: { onConfigure: (t: ConfigTarget) => void }) {
  const { localModels, deleteModel } = useApp();
...
function HardwareTab() {
  const { hardware, saveHardware, detectHardware } = useApp();
        ...
        {gpus.map((g, i) => (
          <div key={i} className="flex items-center gap-2">
            <TextInput
              ...
              onChange={(e) => setGpus((prev) => prev.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)))}
              className="flex-1"
            />
            <TextInput
              type="number"
      ...
      {!s?.reachable && (
        <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-[11px] text-amber-300/90">
          ...
          <p>
            ...
            Discover, copy its <strong>Configure</strong> launch command, and run it on the GPU
            machine. Set <code>LLAMACPP_BASE_URL</code> to point here.
          </p>
        </div>
      )}
    </div>
...
/** The Model Manager: discover/download GGUFs from HuggingFace, see what fits the hardware, get
 *  llama.cpp launch commands, and manage the local model library (the llama.cpp provider). */
export function ModelsPage() {
  const { modelsPageOpen, closeModelsPage } = useApp();
      ...
      <div className="flex h-full flex-col">
        <header className="shrink-0 border-b border-border p-4 pr-12">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Download GGUF models from HuggingFace, see what fits your NVIDIA hardware, and get the
            exact llama-server launch command. Models run on an external llama.cpp server.
          </p>
        </header>
